...
class LocomotionController:
    ...
    def _initialize_gait_files(self):
        ...
        self.steps = toml.load("/home/mi/src/cyberdog_controller/locomotion/Gait_Params_moonwalk.toml")
        ...
        with open("/home/mi/src/cyberdog_controller/locomotion/Gait_Def_moonwalk.toml", 'r') as f:
            self.usergait_msg.data = f.read()
            self.lcm_usergait.publish("user_gait_file", self.usergait_msg.encode())
            f.close()
            time.sleep(0.5)

        with open("Gait_Params_moonwalk_full.toml", 'r') as f:
            self.usergait_msg.data = f.read()
            self.lcm_usergait.publish("user_gait_file", self.usergait_msg.encode())
            f.close()
            time.sleep(0.1)
    ...
    def _load_user_gait_list(self):
        ...
        with open("/home/mi/src/cyberdog_controller/locomotion/Usergait_List.toml", 'r') as f:
    ...
    def set_motion(self, motion_id):
        ...
        if 0 <= motion_id < len(self.steps["step"]):
            ...
            self.my_ctrl.msg.life_count = (self.my_ctrl.msg.life_count + 1) % 128  # 额外递增生命计数（原始逻辑在键盘输入时递增）
        else:
